...
# Baseline to most accurately reflect actual ward
...
PATIENT_COLS = ['Origin', 'Status', 'Acuity', 'Team', 'Bed', 'Admit_Time', 'Off-Service_Time', 'Off-Service_Location', 'Decant_Time', 'Floor_Time', 'Discharge_Order', 'Discharged'] # data to track for each patient
...
def ADMIT_TIME(): 
    ...
    rates = [0.8*i for i in [2.1, 4.2, 1.6, 1.2, 1.2, 1.5]]
...
def simAdmit(patient_ID):
    ...
    if patient_origin == 'NON-MEDICINE':
        patient_df.loc[patient_ID, 'Team'] = 0       
    else:
        patient_df.loc[patient_ID, 'Team'] = TEAM()
    current_patients[patient_df.loc[patient_ID, 'Team']].append(patient_ID)
        
    if patient_status == 'PRIVATE':
        possible_beds = bed_df[np.array(bed_df.Condition=='AVAILABLE') & np.array(bed_df.Status=='PRIVATE') & np.array([bed[0]=='4' and bed_df.loc[bed, 'Nurse'][0:2]=='RN' if patient_acuity=='HIGH' else True for bed in bed_df.index])].index
        if len(possible_beds) > 0:
            simTransfer(patient_ID, np.random.choice(possible_beds)) 
    
    elif patient_status == 'SEMI':
        possible_beds = bed_df[np.array(bed_df.Condition=='AVAILABLE') & np.array([status in ['PRIVATE', 'SEMI'] for status in bed_df.Status]) & np.array([bed[0]=='4' and bed_df.loc[bed, 'Nurse'][0:2]=='RN' if patient_acuity=='HIGH' else True for bed in bed_df.index])].index
        if len(possible_beds) > 0:
            simTransfer(patient_ID, np.random.choice(possible_beds)) 
  
    elif patient_status == 'WARD':
        possible_beds = bed_df[np.array(bed_df.Condition=='AVAILABLE') & np.array(bed_df.Status!='DECANT') & np.array([bed[0]=='4' and bed_df.loc[bed, 'Nurse'][0:2]=='RN' if patient_acuity=='HIGH' else True for bed in bed_df.index])].index
        if len(possible_beds) > 0:
            simTransfer(patient_ID, np.random.choice(possible_beds)) 
        elif patient_origin in ['ED', 'ICU']:
...
def simDecant(patient_ID, bed_ID):                
    bed_df.loc[bed_ID, ['Condition', 'Patient', 'Team']] = ['OCCUPIED', patient_ID, patient_df.loc[patient_ID, 'Team']]
...
def simDischarge(patient_ID):
    ...
    current_patients[patient_df.loc[patient_ID, 'Team']].remove(patient_ID)          
...
def summarize(time_of_day):
    ...
    census_df.loc[sim_time] = [time_of_day] + [sum(emerg_patients.Team==team) for team in [1,2,3]] + [sum(decant_patients.Team==team) for team in [1,2,3]] + [sum(off_service_patients.Team==team) for team in [1,2,3]] + [sum(floor_patients.Team==team) for team in [1,2,3]] + [sum(non_medicine_patients.Floor_Time.notnull())] 
...
for run in range(1, NUM_SIMS+1):
    ...
    next_team = 0 # next team due to receive a patient

    # Populate bed data
    temp = pd.read_csv(PATH + 'Ward1.csv')
    for row in range(len(temp)):
        record = temp.loc[row]
        bed_df.loc[record.ID] = [None for i in range(NUM_BED_COLS)]
        bed_df.loc[record.ID, ['Condition', 'Status', 'Acuity']] = ['AVAILABLE', record.Status, record.Acuity]
    ...
    end = 'v' + str(VERSION) + 'r' + str(run) + '.csv'
    
    file1 = PATH + 'Sim0/patient_df_' + end
    ...
    file2 = PATH + 'Sim0/census_df_' + end
